...
<div class="container my-4 my-lg-5">
    <h2 class="section-title mb-1"><i class="fas fa-magnifying-glass me-2"></i>Search</h2>
    @if ($q !== '')
        <p class="text-muted mb-4">{{ $total }} result{{ $total === 1 ? '' : 's' }} for "<strong>{{ $q }}</strong>"</p>
    @else
        <p class="text-muted mb-4">Type in the search bar to find food, skills, and people.</p>
    @endif
    ...
    @if ($q !== '' && $total === 0)
        <div class="tg-empty">
            ...
            <p class="text-muted mb-0">Try a different keyword or browse <a href="{{ route('food.index') }}">food</a> and <a href="{{ route('skills.index') }}">skills</a>.</p>
        </div>
    @endif

    {{-- People --}}
    @if ($people->isNotEmpty())
        <h5 class="fw-bold mb-3"><i class="fas fa-user-group text-success me-2"></i>People</h5>
    ...
    {{-- Food --}}
    @if ($food->isNotEmpty())
        <h5 class="fw-bold mb-3"><i class="fas fa-utensils text-warning me-2"></i>Food</h5>
        <div class="row g-3 mb-5">
                ...
                <div class="col-md-6 col-lg-3">
                    <a href="{{ route('food.show', $post) }}" class="text-decoration-none text-dark">
                        <div class="tg-card-post">
                            ...
                            @endif
                            <div class="p-3">
                                <span class="badge bg-warning mb-1">{{ $post->food_type }}</span>
    ...
    @if ($skills->isNotEmpty())
        <h5 class="fw-bold mb-3"><i class="fas fa-lightbulb text-success me-2"></i>Skills</h5>
        <div class="row g-3">
                    ...
                    <a href="{{ route('skills.show', $post) }}" class="text-decoration-none text-dark">
                        <div class="tg-card-post">
                            ...
                            <div class="p-3">
                                ...
                                <h6 class="fw-bold mb-1">{{ $post->title }}</h6>
                                <small class="text-muted">{{ $post->user->name }} · {{ $post->skill_level }}</small>
                            </div>
                        </div>
                    </a>
                </div>
