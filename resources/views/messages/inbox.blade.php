...
<style>
    ...
    .tg-thread.unread { border-left:4px solid var(--tg-coral); background:#f7fbf9; }
    .tg-thread-avatar { width:48px;height:48px;border-radius:50%; background:linear-gradient(135deg,var(--tg-orange),var(--tg-coral));
        ...
        <div class="col-md-9">
            ...
            @else
                @foreach ($threads as $thread)
                    <a href="{{ route('messages.conversation', $thread['user']->id) }}" class="tg-thread {{ $thread['unread'] > 0 ? 'unread' : '' }}">
                        <div class="d-flex gap-3 align-items-center">
                            ...
                            @if ($thread['unread'] > 0)<span class="badge bg-danger rounded-pill">{{ $thread['unread'] }}</span>@endif
                        </div>
                    </a>
                @endforeach
            @endif
