    ...
    <div class="row g-4">
        <div class="col-lg-8">
            ...
            <div class="card">
                <div class="card-body p-4">
                    <div class="d-flex justify-content-between align-items-start mb-2">
                        ...
                        <span class="badge bg-info fs-6">{{ ucfirst($skillPost->skill_level) }}</span>
                    </div>
                    <p class="text-muted">{{ $skillPost->description }}</p>
                    @if ($skillPost->available_times)
                        <p class="mb-0"><i class="fas fa-clock text-success me-2"></i><strong>Available:</strong> {{ $skillPost->available_times }}</p>
                    @endif

                    <hr>
                    <h6 class="fw-bold mb-3">Taught by</h6>
                    <div class="d-flex align-items-center justify-content-between flex-wrap gap-3">
                        <a href="{{ route('profile.show', $skillPost->user) }}" class="d-flex align-items-center gap-3 text-decoration-none">
                            ...
                            <div>
                                ...
                                <small class="text-muted">⭐ {{ number_format($skillPost->user->rating,1) }} ({{ $skillPost->user->total_ratings }} ratings)</small>
                            </div>
                        </a>
                        @if (!$isOwner)
            ...
            @if ($isOwner && $bookings->count())
                <div class="card mt-4">
                    <div class="card-body p-4">
                        <h6 class="fw-bold mb-3"><i class="fas fa-calendar-check me-2 text-success"></i>Session Requests ({{ $bookings->where('status','pending')->count() }} pending)</h6>
                        @foreach ($bookings as $booking)
                            <div class="tg-book-row d-flex flex-wrap justify-content-between align-items-center gap-2">
                                <div>
                                    ...
                                    <span class="tg-status s-{{ $booking->status }} ms-1">{{ $booking->status }}</span>
                                ...
                                <div class="d-flex gap-2">
                                    @if ($booking->status === 'pending')
                                        <form action="{{ route('bookings.accept', $booking) }}" method="POST">@csrf<button class="btn btn-primary btn-sm">Accept</button></form>
                                        <form action="{{ route('bookings.decline', $booking) }}" method="POST">@csrf<button class="btn btn-light btn-sm">Decline</button></form>
                                    @elseif ($booking->status === 'accepted')
                                        <form action="{{ route('bookings.complete', $booking) }}" method="POST">@csrf<button class="btn btn-secondary btn-sm">Mark Completed</button></form>
                                    @endif
                                </div>
                            </div>
                        @endforeach
        ...
        <div class="col-lg-4">
            <div class="card sticky-top" style="top:90px;">
                <div class="card-body p-4">
                    @if ($isOwner)
                        <h6 class="fw-bold mb-3">Manage your skill</h6>
                        <a href="{{ route('skills.edit', $skillPost) }}" class="btn btn-secondary w-100 mb-2"><i class="fas fa-edit me-1"></i>Edit Skill</a>
                        <form action="{{ route('skills.destroy', $skillPost) }}" method="POST" onsubmit="return confirm('Delete this skill?')">
                            @csrf @method('DELETE')
                    ...
                    @else
                        ...
                        <h6 class="fw-bold mb-2"><i class="fas fa-calendar-check text-success me-1"></i>Book a free session</h6>
                        @if ($myBooking && in_array($myBooking->status, ['pending','accepted']))
                            <div class="alert alert-success small mb-2">
                                Your request is <strong>{{ $myBooking->status }}</strong>.
                                @if ($myBooking->status==='accepted') Message your teacher to finalize! @endif
                            </div>
                            <form action="{{ route('bookings.cancel', $myBooking) }}" method="POST">@csrf
                                <button class="btn btn-outline-primary w-100">Cancel Request</button>
                            </form>
                        @else
                            <p class="text-muted small">Send a request with your preferred time. It's completely free!</p>
                            <form action="{{ route('bookings.store', $skillPost) }}" method="POST">
                                @csrf
                                <input type="text" name="preferred_time" class="form-control mb-2" placeholder="Preferred time (e.g. Sat 3 PM)">
                                <textarea name="message" class="form-control mb-2" rows="2" placeholder="Optional message"></textarea>
                                <button class="btn btn-primary w-100"><i class="fas fa-paper-plane me-1"></i>Request Session</button>
                            </form>
                        @endif
                    @endif
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content" style="border-radius:18px;">
            <form action="{{ route('ratings.store') }}" method="POST">@csrf
                <div class="modal-header border-0"><h5 class="modal-title">Rate {{ $skillPost->user->name }}</h5>
